...
```mermaid
flowchart TD
    A[Design study and ethics protocol] --> B[Develop consent materials]
    B --> C[Translate and culturally adapt]
    C --> D[Recruit participants ethically]
    D --> E[Explain study and answer questions]
    E --> F{Does participant consent?}
    F -->|Yes| G[Document consent]
    F -->|No| H[Respect decision; do not include]
    G --> I[Proceed with data collection]
    I --> J{New phase, method, or risk?}
    J -->|Yes| K[Re-consent conversation]
    J -->|No| L[Continue with ongoing check-ins]
    K --> F
    L --> M{Participant wants to withdraw?}
    M -->|Yes| N[Handle withdrawal per protocol]
    M -->|No| O[Continue to completion]
    O --> P[Secure and anonymize data]
    P --> Q[Report results, honoring confidentiality]
```
