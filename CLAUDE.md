...
The `ai-anthropology-toolkit` Python package ships in this repository (`pyproject.toml`, `src/ai_anthro_toolkit/`) with an MCP server exposing the research pipeline as 34 tools: data collection (OpenAlex, CrossRef, PubMed, Google Scholar, Google Trends, Google News, Google Patents, Books Ngram, YouTube search and transcripts, podcast RSS — scraper dependencies via the `[data]` extra), citation formatting (`format_citation` renders a bare DOI in any CSL style via the DOI Foundation's formatter, `format_citation_batch` does a reference list in one call and reports per-DOI failures rather than discarding the rest, `list_citation_styles` finds the exact name; it formats registrar metadata and does not verify it), the 42-lens registry, transcript chunking (fully local), and job-based codebook generation, qualitative coding, thematic analysis, and cross-lens comparison. Networks of coded material (`build_network` turns coding output into a code co-occurrence, speaker-to-code, or lens-agreement graph; `analyze_network` lays it out, colors it by community, and reads it; `view_network` shows it as an interactive MCP App in chat; `export_network` writes GEXF for Gephi and Gephi AI) run on networkx with no Gephi installed; the viewer page is copied verbatim from gephi-ai, which is its canonical source. LLM-dependent stages run in `api` mode (ANTHROPIC_API_KEY set) or `delegated` mode, where the orchestrating model completes work packets via `get_next_batch`/`submit_batch` and the server validates every submitted code against the codebook. Coding is gated under the Friction by Design conventions: `ratify_codebook` records the researcher's confirm-or-revise decision (with a content checksum), and `start_coding_job` refuses a codebook with no matching ratification — the server enforces sequence, not sincerity. Standing checks (`ai_anthro_toolkit.checks`) cover codebooks and coded datasets, and `checks.generated` builds a researcher's own checks from the five commitments `tool-building` settles at Stage 4 — persisted as `instrument-checks.json` beside their data, validated against a fixed answer vocabulary, never executed as code. An unanswered commitment generates nothing, and an answer that cannot be checked from the data alone is recorded as unenforceable with its reason. Install with `pip install -e ".[data]"` (add `chunking` to the extras for local transcript chunking) and register with `claude mcp add ai-anthropology -- python3 -m ai_anthro_toolkit.mcp`. Prompt templates are extracted verbatim from the notebooks; `tests/package/` enforces parity. The data-collection and analysis capabilities remain available as the Colab notebooks in `notebooks/`; the methodology, documents, checks, and citations families are server-side only and have no notebook counterpart.
