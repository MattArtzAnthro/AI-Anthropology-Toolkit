...
```mermaid
flowchart LR
    A[Organize & Code Data] --> B[Draft Findings & Thematic Narrative]
    B --> C[Write Introduction: Pose Question]
    C --> D[State Argument and Outline Structure]
    D --> E[Compose Methods Section]
    E --> F[Write Discussion/Analysis]
    F --> G[Draft Conclusion; Form Abstract & Title]
    G --> H[Submit Manuscript]
```
